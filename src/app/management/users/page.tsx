...
const ROLE_COLORS: Record<string, string> = {
  ...
  tester: "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300",
};
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm dark:text-gray-100">
        ...
        {hasFilters && (
          <button onClick={() => { setSearch(""); setRoleFilter(""); setStatusFilter(""); }} className="text-sm text-green-700 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300">
            Clear filters
          </button>
        )}
      </div>
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-400 dark:text-gray-400">{hasFilters ? "No users match your filters." : "No users found."}</div>
        ) : (
          <>
          {/* Desktop table */}
                  ...
                  return (
                    <tr key={u.id} className={`border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 ${!u.isActive ? "opacity-40" : ""}`}>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400 dark:text-gray-400">{u.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-right">
                        {u.id === currentUserId ? (
                          <span className="text-xs text-gray-400 dark:text-gray-400">You</span>
                        ) : (
            ...
            {filtered.map((u: any) => {
              ...
              return (
                <div key={u.id} className={`border border-gray-200 dark:border-gray-700 rounded-lg p-3 ${!u.isActive ? "opacity-40" : ""}`}>
                  ...
                  {u.id === currentUserId && (
                    <div className="text-right mt-2 border-t border-gray-100 dark:border-gray-700 pt-2">
                      <span className="text-xs text-gray-400 dark:text-gray-400">You</span>
                    </div>
                  )}
                </div>
