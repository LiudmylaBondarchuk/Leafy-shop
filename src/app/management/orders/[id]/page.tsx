...
export default function AdminOrderDetailPage() {
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      ...
      if (json.data) {
        setOrder((prev: any) => prev ? { ...prev, internalNotes } : prev);
        setInternalNotes("");
  ...
  if (!order) return <div className="text-center py-12 text-gray-500 dark:text-gray-400">Order not found</div>;

  return (
    <div>
      {/* Header */}
      <div className="flex flex-wrap items-center gap-4 mb-6">
        <Link href="/management/orders" className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 shrink-0">
          <ArrowLeft className="h-5 w-5" />
        </Link>
        <div className="flex-1 min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100 break-all">Order {order.orderNumber}</h1>
      ...
      {order.status === "returned" && (
        <div className="mb-6 bg-orange-50 dark:bg-orange-900/30 border border-orange-200 dark:border-orange-800 rounded-xl p-4">
          <div className="flex items-start gap-3">
            <AlertTriangle className="h-5 w-5 text-orange-600 dark:text-orange-400 mt-0.5" />
            <div>
              <h3 className="font-semibold text-orange-800 dark:text-orange-300">Return Processed</h3>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="p-5">
            ...
            {order.items?.map((item: any) => (
              <div key={item.id} className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700 last:border-0 text-sm">
                <div>
            ...
            {order.wantsInvoice && (
              <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700 space-y-2">
                <a href={`/api/invoices/${order.id}`} target="_blank" className="text-sm text-green-700 hover:text-green-800 font-medium block">
                  View Invoice
                </a>
                {order.status === "cancelled" && (
                  <a href={`/api/credit-notes/${order.id}`} target="_blank" className="text-sm text-red-600 hover:text-red-700 font-medium block">
                    View Credit Note
                  </a>
                )}
              </div>
          ...
          <Card className="p-5">
            ...
            <div className="text-sm space-y-1 text-gray-600 dark:text-gray-400">
              ...
              {order.wantsInvoice && (
                <div className="border-t border-gray-100 dark:border-gray-700 pt-2 mt-2">
                  <p className="font-medium text-gray-900 dark:text-gray-100 mb-1">Invoice</p>
              ...
              {order.trackingNumber && (
                <div className="border-t border-gray-100 dark:border-gray-700 pt-2 mt-2">
                  <p className="font-medium text-gray-900 dark:text-gray-100 mb-1">Tracking Number</p>
                  <p className="font-mono text-green-700">{order.trackingNumber}</p>
                </div>
              )}
              {order.notes && (
                <div className="border-t border-gray-100 dark:border-gray-700 pt-2 mt-2">
                  <p className="font-medium text-gray-900 dark:text-gray-100 mb-1">Notes</p>
        ...
        <div className="space-y-6">
          ...
          {order.availableTransitions?.length > 0 && (
            <Card className="p-5">
              ...
              <select
                ...
                className="w-full rounded-lg border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-600 mb-3"
              >
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowConfirm(false)}>
              <div className="bg-white dark:bg-gray-800 rounded-xl p-6 max-w-sm mx-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center gap-3 mb-4">
                  <div className={`p-2 rounded-full ${selectedStatus === "cancelled" ? "bg-red-100 dark:bg-red-900/30" : "bg-green-100 dark:bg-green-900/30"}`}>
                    <AlertTriangle className={`h-5 w-5 ${selectedStatus === "cancelled" ? "text-red-600 dark:text-red-400" : "text-green-600 dark:text-green-400"}`} />
                  </div>
                  <h3 className="font-semibold text-gray-900 dark:text-gray-100">Confirm Status Change</h3>
                </div>
              ...
              {order.statusHistory?.map((entry: any, i: number) => (
                <div key={i} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div className="w-6 h-6 rounded-full bg-green-100 dark:bg-green-900/30 text-green-700 flex items-center justify-center">
                      <Check className="h-3 w-3" />
                    </div>
                    {i < order.statusHistory.length - 1 && <div className="w-0.5 h-4 bg-gray-200 dark:bg-gray-700 mt-1" />}
                  </div>
                  <div>
          ...
          <Card className="p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              {order.internalNotes && !editingNotes && (
                <button
                  onClick={() => { setInternalNotes(order.internalNotes); setEditingNotes(true); }}
            ...
            {order.internalNotes && !editingNotes ? (
              <p className="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{order.internalNotes}</p>
            ) : (
              <>
                <textarea
                  value={internalNotes}
                  ...
                  rows={4}
                  className="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-600 mb-3"
                ...
                <div className="flex gap-2">
                  {editingNotes && (
                    <Button
                      ...
                      size="sm"
                      className="flex-1"
                      onClick={() => { setInternalNotes(""); setEditingNotes(false); }}
                    >
                      Cancel
                    </Button>
                  )}
