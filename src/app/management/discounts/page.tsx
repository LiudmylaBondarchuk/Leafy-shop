  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Discount Codes</h1>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        <div className="relative">
          ...
          <input
            ...
            className="rounded-lg border border-gray-300 dark:border-gray-600 pl-9 pr-3 py-2 text-sm w-full sm:w-48 dark:bg-gray-800 dark:text-gray-100"
          />
        </div>
        ...
        <input
          ...
          placeholder="From"
          className="rounded-lg border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm dark:bg-gray-800 dark:text-gray-100"
          title="Valid from"
        />
        <input
          type="date"
        ...
        />
        {hasFilters && (
          <button onClick={() => { setSearch(""); setTypeFilter(""); setStatusFilter(""); setDateFrom(""); setDateTo(""); }} className="text-sm text-green-700 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300">
            Clear filters
          </button>
        )}
      </div>
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-400 dark:text-gray-400">{hasFilters ? "No discount codes match your filters." : "No discount codes yet."}</div>
        ) : (
          <>
          {/* Desktop table */}
            ...
            <table className="w-full text-sm min-w-[800px]">
              <thead className="bg-gray-50 dark:bg-gray-800">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Min Order</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Usage</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Valid</th>
              ...
              <tbody>
                {filtered.map((code: any) => {
                  ...
                  return (
                    <tr key={code.id} className={`border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 ${!code.isActive ? "opacity-40" : ""}`}>
                      <td className="px-4 py-3">
                        <span className="font-mono font-medium text-gray-900 dark:text-gray-100">{code.code}</span>
                        {code.description && <p className="text-xs text-gray-400 dark:text-gray-400 mt-0.5">{code.description}</p>}
                      </td>
                      <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{getTypeLabel(code.type)}</td>
          ...
          <div className="sm:hidden space-y-3 p-3">
            {filtered.map((code: any) => {
              const status = getStatus(code);
              return (
                <div key={code.id} className={`border border-gray-200 dark:border-gray-700 rounded-lg p-3 ${!code.isActive ? "opacity-40" : ""}`}>
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <span className="font-medium text-gray-900 dark:text-gray-100">{getValueLabel(code)}</span>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setDeleteModal(null)}>
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 max-w-sm mx-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-semibold text-gray-900 dark:text-gray-100">Delete Discount Code</h3>
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
