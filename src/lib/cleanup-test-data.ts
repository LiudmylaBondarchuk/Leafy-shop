...
export interface CleanupSummary {
  ...
  discounts: number;
  orders: number;
  revertedStatuses: number;
  auditLogs: number;
  lowStockAlerts: number;
}

export async function runTestDataCleanup(): Promise<CleanupSummary> {
  ...
  try {
    ...
    if (recipients.length > 0) {
      ...
      if (lowStockProducts.length > 0) {
        ...
        console.log(`[CLEANUP] Low stock alert sent for ${lowStockProducts.length} products to ${recipients.join(", ")}`);
      }
    }
  } catch (err) {
    console.error("[CLEANUP] Low stock alert error:", err instanceof Error ? err.message : "Unknown error");
  }
...
async function sendLowStockAlert(
  ...
  if (!resend) {
    console.warn("[CLEANUP] Resend not configured, skipping low stock alert email");
    return;
  }
